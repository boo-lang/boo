...
function CurLangList(docLangList){
	...
	// (Try 1) If it has a bookmark, check to see if Language is near A Name
	if (bookmarkStart > -1){
		// Results in a single-item langList
		var bookmark = address.substring(bookmarkStart + 1);
		tempLangList = new Array(1);
		tempLangList[0] = findLanguage(bookmark);
	}

	// (Try 2) Check the dynamic help window for current language.
	...
	// (Try 3) Read the persisted value 
	if (tempLangList == null){
		// Results in a single-item langList
		if ( this.cook != null ) {
			if (this.cook.getValue("lang.all") != "1") {
				var lang = this.cook.getValue("lang");
				var found = false;
...
function declareFilterTopicForLang(langName, saveLang){
	...
	if (saveLang && isLangPersistable(langName)) {
		if (this.cook != null) {
			this.cook.putValue('lang', langName);
			this.cook.putValue('lang.all', '');
		}
	}
