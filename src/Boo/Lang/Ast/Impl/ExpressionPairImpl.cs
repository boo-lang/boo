...
{	
	...
	public abstract class ExpressionPairImpl : Node
	{
		...
		{
			return Clone() as ExpressionPair;
		}
		...
		{
			ExpressionPair clone = FormatterServices.GetUninitializedObject(typeof(ExpressionPair)) as ExpressionPair;
			...
			if (null != _first)
			{
				clone._first = _first.CloneNode();
				clone._first.InitializeParent(clone);
			}

			if (null != _second)
			{
				clone._second = _second.CloneNode();
