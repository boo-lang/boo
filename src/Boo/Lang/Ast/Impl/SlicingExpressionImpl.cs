...
{	
	...
	public abstract class SlicingExpressionImpl : Expression
	{
		...
		{
			SlicingExpression clone = FormatterServices.GetUninitializedObject(typeof(SlicingExpression)) as SlicingExpression;
			...
			if (null != _begin)
			{
				clone._begin = _begin.CloneNode();
				clone._begin.InitializeParent(clone);
			}

			if (null != _end)
			{
				clone._end = _end.CloneNode();
				clone._end.InitializeParent(clone);
			}

			if (null != _step)
			{
				clone._step = _step.CloneNode();
