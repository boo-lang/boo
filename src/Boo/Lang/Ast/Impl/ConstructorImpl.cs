...
{	
	...
	public abstract class ConstructorImpl : Method
	{
		...
		override public object Clone()
		{
			...
			if (null != _parameters)
			{
				clone._parameters = _parameters.CloneNode();
				clone._parameters.InitializeParent(clone);
			}

			if (null != _returnType)
			{
				clone._returnType = _returnType.CloneNode();
				clone._returnType.InitializeParent(clone);
			}

			if (null != _returnTypeAttributes)
			{
				clone._returnTypeAttributes = _returnTypeAttributes.CloneNode();
				clone._returnTypeAttributes.InitializeParent(clone);
			}

			if (null != _body)
			{
				clone._body = _body.CloneNode();
				clone._body.InitializeParent(clone);
			}

			if (null != _locals)
			{
				clone._locals = _locals.CloneNode();
