    ...
    internal class BooParserAstBuilderListener : AbstractParseTreeVisitor<Node>, IBooParserVisitor<Node>
    {
        ...
        Import VisitImport_directive_(BooParser.Import_directive_Context context)
        {
            ...
            if (context.FROM() != null)
            {
                ...
                {
                    text = context.SINGLE_QUOTED_STRING().GetText();
                    li = GetLexicalInfo(context.SINGLE_QUOTED_STRING());
                }
                else
                {
                    text = context.DOUBLE_QUOTED_STRING().GetText();
        ...
        Expression VisitChar_literal(BooParser.Char_literalContext context)
        {
            ...
            if (t != null)
                return new CharLiteralExpression(GetLexicalInfo(t), t.GetText());
            var i = context.INT();
            if (i != null)
        ...
        Expression VisitString_literal(BooParser.String_literalContext context)
        {
            ...
            if (dqs != null)
            {
                e = new StringLiteralExpression(GetLexicalInfo(dqs), dqs.GetText());
                e.Annotate("quote", "\"");
                return e;
            }
            ...
            if (sqs != null)
            {
                e = new StringLiteralExpression(GetLexicalInfo(sqs), sqs.GetText());
                e.Annotate("quote", "'");
                return e;
            }
            ...
            if (tqs != null)
            {
                e = new StringLiteralExpression(GetLexicalInfo(tqs), tqs.GetText());
                e.Annotate("quote", "\"\"\"");
                return e;
            }
            var bqs = context.BACKTICK_QUOTED_STRING();
            e = new StringLiteralExpression(GetLexicalInfo(bqs), bqs.GetText());
            e.Annotate("quote", "`");
            return e;
        }
