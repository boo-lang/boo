		...
		{    // ( ... )*
			for (;;)
			{
				if (((cached_LA1=='*') && ((cached_LA2 >= '\u0003' && cached_LA2 <= '\ufffe')) && ((LA(3) >= '\u0003' && LA(3) <= '\ufffe')))&&( LA(2) != '/' ))
				{
					...
					bool synPredMatched784 = false;
					if (((cached_LA1=='/') && (cached_LA2=='*') && ((LA(3) >= '\u0003' && LA(3) <= '\ufffe'))))
					{
						int _m784 = mark();
						synPredMatched784 = true;
						inputState.guessing++;
						try {
							{
								match("/*");
						...
						{
							synPredMatched784 = false;
						}
						rewind(_m784);
						inputState.guessing--;
					}
					if ( synPredMatched784 )
					{
						mML_COMMENT(false);
					}
	...
	protected void mTRIPLE_QUOTED_STRING(bool _createToken) //throws RecognitionException, CharStreamException, TokenStreamException
{
		...
		{    // ( ... )*
			...
			{
				// nongreedy exit test
				if ((cached_LA1=='"') && (cached_LA2=='"') && (LA(3)=='"')) goto _loop760_breakloop;
	...
	public void mX_RE_LITERAL(bool _createToken) //throws RecognitionException, CharStreamException, TokenStreamException
{
		...
		{ // ( ... )+
			...
			for (;;)
			{
				...
				{
					if (_cnt793 >= 1) { goto _loop793_breakloop; } else { throw new NoViableAltForCharException(cached_LA1, getFilename(), getLine(), getColumn());; }
				}
